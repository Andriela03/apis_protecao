...
try:
    ...
    verificar(
        www_auth is not None and "Digest" in www_auth,
        "Estratégia Digest identificada",
        "Estratégia Digest não encontrada"
    )

    verificar(
        www_auth is not None and 'realm="hello"' in www_auth,
        "Realm identificado",
        "Realm inesperado"
    )

    verificar(
        www_auth is not None and "nonce=" in www_auth,
        "Nonce enviado pelo servidor",
        "Nonce ausente"
    )

    verificar(
        www_auth is not None and "qop=" in www_auth,
        "qop enviado pelo servidor",
        "qop ausente"
    )
...
# ==========================================
# TESTE 2
# REQUISIÇÃO COM DIGEST AUTH
# ==========================================

print("\n\n" + "=" * 60)
print("TESTE 2 - ACESSO COM DIGEST AUTH")
print("=" * 60)
...
try:
    ...
    verificar(
        authorization is not None and authorization.startswith("Digest "),
        "Cabeçalho Digest enviado corretamente",
        "Cabeçalho Authorization não utiliza Digest"
    )

    verificar(
        authorization is not None and 'username="admin"' in authorization,
        "Usuário enviado no desafio Digest",
        "Usuário não encontrado no cabeçalho Digest"
    )
